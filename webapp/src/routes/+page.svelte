<script>
  ...
  const runtimeOptions = [
    {
      ...
      placeholder:
        "https://<dotnet-app>.azurewebsites.net/api/DotnetFunction?code=YOUR_CODE",
      workloads: [
        {
          id: "cpu",
          name: "CPU intensive",
    ...
    },
    {
      ...
      placeholder:
        "https://<node-app>.azurewebsites.net/api/NodeFunction?code=YOUR_CODE",
      workloads: [
        {
          id: "cpu",
          name: "CPU intensive",
    ...
    },
    {
      ...
      placeholder:
        "https://<python-app>.azurewebsites.net/api/PythonFunction?code=YOUR_CODE",
      workloads: [
        {
          id: "cpu",
          name: "CPU intensive",
    ...
    },
    {
      ...
      placeholder:
        "https://<ps-app>.azurewebsites.net/api/PowerShellFunction?code=YOUR_CODE",
      workloads: [
        {
          id: "cpu",
          name: "CPU intensive",
    ...
    },
    {
      ...
      placeholder:
        "https://<java-app>.azurewebsites.net/api/JavaFunction?code=YOUR_CODE",
      workloads: [
        {
          id: "cpu",
          name: "CPU intensive",
  ...
  onMount(async () => {
    ...
    const configured = runtimeOptions.filter((r) =>
      (urlMap[r.id] ?? "").trim(),
    );
  ...
  $: configuredRuntimes = runtimeOptions.filter((r) =>
    (urlMap[r.id] ?? "").trim(),
  );
  ...
  $: selectedRuntime = configuredRuntimes.find(
    (r) => r.id === selectedRuntimeId,
  );

  $: if (
    selectedRuntime &&
    !selectedRuntime.workloads.find((w) => w.id === selectedWorkloadId)
  ) {
    selectedWorkloadId = selectedRuntime.workloads[0]?.id ?? "";
  }
  $: selectedWorkload = selectedRuntime?.workloads.find(
    (w) => w.id === selectedWorkloadId,
  );
  ...
  $: currentBaseUrl =
    (selectedRuntime && urlMap[selectedRuntime.id]?.trim()) || "";
  $: currentUrl =
    selectedRuntime && selectedWorkload && currentBaseUrl
      ? appendParams(currentBaseUrl, {
          workload: selectedWorkload.id,
          ...(selectedWorkload.params ?? {}),
        })
      : "";

  $: maxDuration = recentRequests.reduce(
    (acc, req) => Math.max(acc, req.durationMs ?? 0),
    0,
  );
  $: timelineRequests = [...recentRequests].sort(
  ...
  $: seriesData = (() => {
    ...
    const series = Array.from(grouped.entries()).map(([runtimeId, entries]) => {
      const points = entries.map((req, idx) => {
        ...
        const y =
          height - pad - ((req.durationMs ?? 0) / maxVal) * (height - 2 * pad);
        return {
          x,
          y,
      ...
      return {
        runtimeId,
        runtimeName:
          runtimeOptions.find((r) => r.id === runtimeId)?.name ?? runtimeId,
  ...
  async function runFunction() {
    ...
    if (!currentUrl || currentUrl.includes("YOUR_CODE")) {
      error =
        "Configure a valid signed Azure Function URL in the setup dialog first.";
      showConfig = true;
      return;
    }
    ...
    message = result.ok
      ? "Request completed"
      : `Request returned status ${result.status}`;
    if (!result.ok && !error) {
  ...
  async function runAutomaticTests() {
    ...
    const results = await Promise.all(
      tasks.map(async (t) => performCall(t.runtime, t.workload, t.url)),
    );
  ...
  function calculateAverage() {
    ...
    averageByRuntime = runtimeOptions
      .map((runtime) => {
        const entries = recentRequests.filter(
          (req) => req.runtimeId === runtime.id,
        );
        if (!entries.length) return null;
        const total = entries.reduce(
          (sum, req) => sum + (req.durationMs ?? 0),
          0,
        );
        return {
          runtimeId: runtime.id,
...
<div class="min-h-screen flex flex-col">
  <main class="text-slate-100 px-4 py-8 md:py-12 flex-1">
    <div class="max-w-7xl mx-auto space-y-8">
      <section class="glass-panel grid-bg rounded-3xl p-6 md:p-8">
        ...
        >
          <div class="space-y-2">
            ...
            <h1
              class="text-3xl md:text-4xl font-semibold font-display tracking-tight"
            >
              Runtime latency comparison
            </h1>
            <p class="text-slate-300 max-w-2xl">
              Test .NET, Node.js, Python, PowerShell, and Java workloads side by
              side. Enter one signed Function URL per runtime, then switch
              workloads via query params automatically.
            </p>
          </div>
          <div
            class="glass-panel rounded-2xl p-4 border border-cyan-400/30 shadow-neon space-y-1"
          >
            <p class="text-sm text-slate-300">Configuration</p>
            <div
              class={`text-2xl font-semibold ${configuredRuntimes.length ? "text-emerald-300" : "text-amber-200"}`}
            >
              {configuredRuntimes.length
                ? `${configuredRuntimes.length} runtime(s) set`
                : "Add URLs"}
            </div>
      ...
      <section class="grid gap-6 lg:grid-cols-[1fr_1fr]">
        <div class="glass-panel rounded-3xl p-6 md:p-7 space-y-6">
          <div class="flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
              ...
              <span
                class="px-3 py-1 rounded-full bg-emerald-500/15 text-emerald-300 text-xs border border-emerald-400/30"
              >
                Frontend only
              </span>
            </div>
          ...
          {#if configuredRuntimes.length === 0}
            <div
              class="rounded-2xl border border-dashed border-white/10 bg-white/5 p-5 text-sm text-slate-300"
            >
              ...
              <div class="space-y-3">
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-xs text-slate-400"
                    >Uses query params on your single Function URL</span
                  >
                </div>
                  ...
                  {#each selectedRuntime.workloads as workload}
                    ...
                    >
                      <div class="flex items-start justify-between gap-3">
                        ...
                        <span
                          class="text-[11px] px-2 py-1 rounded-full bg-white/5 text-slate-300 border border-white/10"
                        >
            ...
            <div class="space-y-2">
              <div class="flex items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-xs text-slate-400"
                    >One per runtime (stored locally)</span
                  >
                </div>
                <button
              ...
              <div
                class="rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs text-slate-200 break-all"
              >
                {currentBaseUrl ||
                  "Not set. Open setup to add your signed Function URL."}
              </div>
            </div>
        ...
        <div
          class="glass-panel rounded-3xl p-6 md:p-7 space-y-5 border border-white/10"
        >
          <div class="flex items-center justify-between gap-4">
            <div>
              <p class="text-xs uppercase tracking-[0.2em] text-slate-400">
            ...
            <span class="text-xs text-slate-400"
              >Last {recentRequests.length} requests</span
            >
          </div>
          ...
          <div class="space-y-2">
            ...
            <div
              class="relative h-16 rounded-2xl border border-white/10 bg-white/5 overflow-hidden"
            >
          ...
          <div class="space-y-2">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-sm text-slate-300">
                  Y: ms • X: run count per runtime
                </p>
              </div>
              <div class="flex flex-wrap gap-2 text-xs">
                {#each seriesData.series as s}
                  <span
                    class="flex items-center gap-1 px-2 py-1 rounded-full border border-white/10 bg-white/5"
                    style={`color:${s.color}; border-color:${s.color}33`}
                  >
                    <span
            ...
            <div
              class="rounded-2xl border border-white/10 bg-white/5 p-3 relative"
            >
              {#if seriesData.series.length === 0}
                <div
                  class="h-40 flex items-center justify-center text-sm text-slate-400"
                >
                  Run a request to see the graph
                </div>
              {:else}
                <svg
                ...
                {#if hoveredPoint}
                  ...
                  >
                    <div
                      class="rounded-xl border border-white/20 bg-black/80 text-[11px] px-3 py-2 text-slate-100 shadow-lg backdrop-blur-sm"
                    >
                      <div class="font-semibold">
                        {hoveredPoint.runtimeName}
                      </div>
                      <div class="text-slate-300">
                        {hoveredPoint.workloadName} • Run #{hoveredPoint.runIndex}
                      </div>
                      <div class="text-cyan-200">
                        {formatDuration(hoveredPoint.duration)}
                      </div>
                    </div>
                  </div>
                {/if}
          ...
          {#if recentRequests[0]}
            <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div class="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p class="text-sm text-slate-300">
                    Per runtime (from recent requests)
                  </p>
                </div>
                <span class="text-xs text-slate-400"
                  >{recentRequests.length} total</span
                >
              </div>
              {#if averageByRuntime.length === 0}
                <div class="mt-3 text-sm text-slate-400">
                  Run a request to see averages.
                </div>
              {:else}
                <div class="mt-3 grid gap-2">
                  {#each averageByRuntime as avg}
                    <div
                      class="flex items-center justify-between rounded-xl border border-white/5 bg-black/20 px-3 py-2"
                    >
                      <span class="text-sm text-slate-100"
                        >{avg.runtimeName}</span
                      >
                      <span class="text-sm text-cyan-100"
                        >{formatDuration(avg.avgDurationMs)} · {avg.count} runs</span
                      >
                    </div>
                  {/each}
                </div>
            ...
            <div
              class="rounded-2xl border border-cyan-400/30 bg-cyan-400/10 p-4 shadow-neon"
            >
              <p class="text-xs text-cyan-100 uppercase tracking-[0.2em]">
                Latest run
              </p>
              <div class="flex items-center justify-between gap-3">
                <div>
                  <p class="text-sm text-cyan-50">
                    {recentRequests[0].runtimeName} | {recentRequests[0]
                      .workloadName}
                  </p>
                  <p class="text-xs text-slate-200/80">
                    {formatTime(recentRequests[0].timestamp)}
                  </p>
                </div>
                <div class="text-right">
                  <p
                    class="text-lg font-semibold text-slate-900 px-3 py-1 rounded-xl bg-white/80 inline-block"
                  >
                    {formatDuration(recentRequests[0].durationMs)}
                  </p>
                  <p class="text-[11px] text-cyan-50 mt-1">
        ...
        {:else}
          <div class="grid gap-3">
            {#each recentRequests as req}
              <div
                class="rounded-2xl border border-white/10 bg-white/5 p-4 md:p-5 flex flex-col gap-3"
              >
                ...
                {#if req.bodySnippet}
                  <p class="text-xs text-slate-400 max-h-12 overflow-hidden">
                    Response: {req.bodySnippet}
                  </p>
                {/if}
              </div>
            {/each}
    ...
    {#if showConfig}
      <div
        class="fixed inset-0 bg-black/70 backdrop-blur-sm z-[200] flex items-center justify-center px-4"
      >
        <div
          class="max-h-[90vh] overflow-y-auto w-full max-w-4xl glass-panel rounded-3xl p-6 md:p-7 border border-cyan-400/40 space-y-6"
        >
          <div class="flex items-center justify-between gap-3">
            <div>
              <p class="text-xs uppercase tracking-[0.2em] text-slate-400">
              ...
              <h3 class="text-xl font-semibold">
                Paste one signed Function URL per runtime
              </h3>
              <p class="text-sm text-slate-300">
                If left empty, that runtime will be hidden on the main screen.
              </p>
            </div>
            <button
            ...
            {#each runtimeOptions as runtime}
              ...
              >
                ...
                <input
                  ...
                  on:input={(e) =>
                    updateWizardUrl(runtime.id, e.currentTarget.value)}
                />
              </div>
            {/each}
          </div>
    ...
    {/if}
    <section class="max-w-7xl mx-auto mt-12 md:mt-16">
      <div
        class="glass-panel rounded-3xl p-8 md:p-10 bg-gradient-to-b from-slate-900/70 to-slate-900/40 border border-white/5"
      >
        <div class="text-center space-y-3">
          ...
          <p class="text-sm md:text-base text-slate-300">
            Built with &hearts; by a passionate developer. Get help and explore
            more projects.
          </p>
        </div>
        ...
        <div class="mt-8 grid gap-5 md:gap-6 md:grid-cols-2">
          <div
            class="rounded-3xl bg-slate-700/50 border border-white/10 p-6 md:p-8 shadow-lg space-y-4"
          >
            <h4 class="text-2xl font-semibold text-white">
              About the Developer
            </h4>
            <p class="text-slate-200">
              LNC App Accountlinks is crafted and maintained with dedication by <strong
                >Luca Noah Caprez</strong
              >.
            </p>
            <a
          ...
          >
            ...
            <div class="space-y-3">
              ...
              >
                <span aria-hidden="true">💬</span>
                <span class="font-semibold">Community & Contact</span>
              </a>
              <a
  ...
  <footer class="mt-auto">
    <div
      class="flex flex-wrap items-center justify-center bg-white/5 border-t border-white/10 text-xs md:text-sm py-4 text-slate-200"
    >
      <p
        class="flex flex-wrap items-center justify-center gap-2 px-4 text-center"
      >
        ...
        <span class="hidden sm:inline">
          ...
          <a
            href="https://lucanoahcaprez.ch/me"
            target="_blank"
            class="text-cyan-200 hover:text-white font-semibold"
            >@lucanoahcaprez</a
          >
        </span>
        |
        <a
          href="https://lucanoahcaprez.ch/legal"
          target="_blank"
          class="text-cyan-200 hover:text-white font-semibold">Legal</a
        >
      </p>
    </div>
  </footer>
